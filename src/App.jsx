...
const ukMenu = [
  ...
  },
  {
    ...
    items: [
      ...
      ["Baja fish", "£10", "Crisp fish, slaw, crema and salsa."],
      [
  ...
  },
  {
    title: "Quesadillas",
    items: [
      ["Build your own", "£11", "Cheese and your choice of filling."],
      [
        "Quesabirria",
        "£12",
  ...
  },
  {
    title: "Sweet",
    items: [
      ["Tres leches", "£8", "Soft sponge, cream and cinnamon."],
      ["Churros", "£7", "Cinnamon sugar and honey caramel."],
    ],
  },
];
...
const mxMenu = [
  ...
  },
  {
    title: "Quesadillas",
    items: [
      ["Arma la tuya", "$185", "Quesillo y el relleno que elijas."],
      [
        "Quesabirria",
        "$205",
  ...
  },
  {
    title: "Postres",
    items: [
      ["Tres leches", "$135", "Bizcocho suave, crema y canela."],
      ["Churros", "$125", "Azúcar, canela y caramelo de miel."],
    ],
  },
];
...
function getRoute(pathname) {
  ...
  const isMexico = clean === "/mx" || clean.startsWith("/mx/");
  const region = isMexico ? "mx" : "uk";
  const base = regions[region].base;

  const relative =
    clean === base || clean === "/" ? "/" : clean.slice(base.length) || "/";
...
function Button({ to, navigate, children, secondary = false }) {
  return (
    <AppLink
      to={to}
      navigate={navigate}
...
function ImagePanel({
  ...
  return (
    <div
      className={`image-panel${className ? ` ${className}` : ""}${
        contain ? " image-panel--contain" : ""
      }`}
    >
      <img
        src={src}
...
function Header({ route, navigate }) {
  ...
  const switchTo = route.region === "uk" ? "/mx" : "/";
    ...
    <header className="site-header">
      ...
      <nav className={`nav${menuOpen ? " nav--open" : ""}`}>
        {pages.map(([label, slug]) => {
          const path = routePath(route.region, slug);
        ...
        <AppLink
          ...
          ariaLabel={
            route.region === "uk"
              ? "Switch to Mexico site"
              : "Switch to UK site"
          }
        >
          {route.region === "uk" ? "MX" : "UK"}
        </AppLink>
      </nav>
    </header>
  );
  ...
  return (
    <footer className="footer">
      ...
      <div className="footer__links">
        ...
        <a
          href={region === "uk" ? UK_INSTAGRAM : MX_INSTAGRAM}
          target="_blank"
          rel="noreferrer"
        >
          Instagram
      ...
      <div className="footer__bottom">
        <span>© {new Date().getFullYear()} Benditos.</span>
...
function PageHero({ eyebrow, title, copy, src, label, position = "center" }) {
  return (
...
function LaunchSection({ region, navigate }) {
  ...
  return (
    <section className="launch-block section">
      ...
      <div className="actions">
        <a
          className="button"
          href={isMexico ? MX_INSTAGRAM : UK_INSTAGRAM}
          target="_blank"
          rel="noreferrer"
        >
          Instagram
        ...
        <Button
          to={routePath(region, "contact")}
          navigate={navigate}
          secondary
        >
          {isMexico ? "Recibir novedades" : "Get updates"}
        </Button>
      </div>
    </section>
  );
...
function HomePage({ region, navigate }) {
  const copy = regions[region];
  const isMexico = region === "mx";
  ...
  return (
    <main>
      ...
      <section className="marquee">
        {(isMexico
          ? ["BRASAS", "MAÍZ", "CÍTRICOS", "CHILE", "MIEL"]
          : ["FIRE", "CORN", "CITRUS", "CHILLI", "HONEY"]
        ).map((item) => (
          <span key={item}>
            {item}
            <i>✦</i>
      ...
      <section className="section intro-grid">
        <div>
          <p className="eyebrow">
            {isMexico ? "Nuestra manera de cocinar" : "Our approach"}
          </p>
      ...
      <section className="split-feature section">
        ...
        <div className="split-feature__content">
          <p className="eyebrow">
            {isMexico ? "Hecho para compartir" : "Made for sharing"}
          </p>
      ...
      <section className="honey-banner section">
        <div>
          <p className="eyebrow">
            {isMexico ? "Miel mexicana" : "Mexican honey"}
          </p>

          <h2>
            {isMexico ? "No sólo para el postre." : "Not just for dessert."}
          </h2>
...
function MenuPage({ region }) {
  const isMexico = region === "mx";
  const menu = isMexico ? mxMenu : ukMenu;

  return (
    <main>
      <PageHero
        eyebrow={isMexico ? "Benditos Mexicali" : "Benditos London"}
        title={isMexico ? "Menú" : "Menu"}
        copy={
          isMexico
      ...
      <section className="menu-section section">
        <div className="menu-notice">
          <strong>{isMexico ? "Menú de lanzamiento" : "Launch menu"}</strong>
        ...
        <div className="menu-columns">
          {menu.map((section) => (
            <article className="menu-group" key={section.title}>
              <h2>{section.title}</h2>
...
function HoneyPage({ region, navigate }) {
  const isMexico = region === "mx";
  ...
  return (
    <main>
      <PageHero
        eyebrow={isMexico ? "Un ingrediente central" : "A central ingredient"}
        title={isMexico ? "Nuestra miel" : "Honey"}
        copy={
          isMexico
      ...
      <section className="section intro-grid">
        <div>
          <p className="eyebrow">
            {isMexico ? "Más que dulzor" : "More than sweetness"}
          </p>
      ...
      <section className="split-feature section">
        ...
        <div className="split-feature__content">
          <p className="eyebrow">
            {isMexico ? "De la cocina a tu casa" : "From our kitchen to yours"}
          </p>
...
function MelodisaPage({ region, navigate }) {
  const isMexico = region === "mx";
  ...
  return (
    <main className="melodisa-page">
      <section className="melodisa-intro">
        <div className="melodisa-intro__image">
          <img
            ...
            alt={
              isMexico
                ? "Botellas de hidromiel Melodisa"
                : "Bottles of Melodisa Mexican mead"
            }
          />
        </div>

        <div className="melodisa-intro__content">
          <div className="melodisa-logo-frame">
            <img
              src={melodisaLogo}
          ...
          <p className="eyebrow">
            {isMexico ? "Hidromiel mexicana" : "Mexican mead"}
          </p>

          <h1>
            {isMexico
              ? "Una tradición antigua, hecha en México."
              : "Ancient tradition, made in Mexico."}
          </h1>

          <p className="melodisa-intro__lead">
            {isMexico
              ? "Melodisa transforma miel mexicana en hidromiel expresiva y premiada, elaborada con paciencia, imaginación y respeto por cada ingrediente."
              : "Melodisa transforms Mexican honey into expressive, award-winning mead made with patience, imagination and respect for every ingredient."}
          </p>

          <p>
            {isMexico
              ? "Una bebida histórica llevada al presente mediante miel cuidadosamente seleccionada, fruta, botánicos y sabores profundamente mexicanos."
              : "A historic drink brought into the present through carefully selected honey, fruit, botanicals and distinctly Mexican flavours."}
          </p>
          ...
          <div className="melodisa-actions">
            <a
              ...
              className="button"
            >
              {isMexico ? "Conocer Melodisa" : "Discover Melodisa"}
            </a>
            ...
            <Button
              ...
              secondary
            >
              {isMexico ? "Explorar nuestra miel" : "Explore our honey"}
            </Button>
          </div>
        </div>
      </section>
      ...
      <section className="melodisa-statement">
        <p className="eyebrow">
          {isMexico
            ? "Miel, tiempo y fermentación"
            : "Honey, time and fermentation"}
        </p>

        <h2>
          {isMexico
            ? "Una bebida definida por el carácter de la miel mexicana."
            : "A drink shaped by the character of Mexican honey."}
        </h2>

        <p>
          {isMexico
            ? "Cada lote comienza con miel. Su aroma, origen y complejidad natural forman la base de una bebida situada entre el vino, la tradición y la experimentación."
            : "Every batch begins with honey. Its aroma, origin and natural complexity form the foundation of a drink that sits between wine, tradition and experimentation."}
        </p>
      </section>
      ...
      <section className="melodisa-feature">
        <div className="melodisa-feature__content">
          <p className="eyebrow">
            {isMexico ? "De la colmena a la botella" : "From hive to bottle"}
          </p>

          <h2>
            {isMexico
              ? "Elaborada lentamente y con intención."
              : "Made slowly and with intention."}
          </h2>

          <p>
            {isMexico
              ? "La miel se mezcla con agua y fermenta cuidadosamente, permitiendo que sus características florales, frutales y terrosas se desarrollen de forma natural."
              : "Honey is blended with water and fermented carefully, allowing its floral, fruity and earthy characteristics to develop naturally."}
          </p>

          <p>
            {isMexico
              ? "El resultado puede ser brillante y fresco, profundo y cálido o delicadamente aromático."
              : "The result can be bright and fresh, rich and warming or delicately aromatic."}
          </p>
        </div>
        ...
        <div className="melodisa-feature__image">
          <img
            ...
            alt={
              isMexico
                ? "Hidromiel Melodisa Sonata"
                : "Melodisa Sonata Mexican mead"
            }
          />
        </div>
      </section>
      ...
      <section className="melodisa-award">
        <div className="melodisa-award__copy">
          ...
          <h2>
            {isMexico
              ? "Hidromiel mexicana premiada."
              : "Award-winning Mexican mead."}
          </h2>

          <p>
            {isMexico
              ? "Melodisa ha recibido reconocimiento por la calidad, personalidad y elaboración de sus hidromieles."
              : "Melodisa has received recognition for the quality, personality and craftsmanship of its meads."}
          </p>
        </div>
        ...
        <div className="melodisa-award__image">
          <img
            ...
            alt={
              isMexico
                ? "Melodisa recibiendo un premio"
                : "Melodisa receiving an award"
            }
          />
        </div>
      </section>
      ...
      <section className="melodisa-cta">
        ...
        <h2>
          {isMexico
            ? "Miel con origen, sabor y propósito."
            : "Honey with origin, flavour and purpose."}
        </h2>

        <p>
          {isMexico
            ? "Melodisa representa la creatividad, el conocimiento y la elaboración detrás de los productores mexicanos que queremos celebrar."
            : "Melodisa represents the creativity, knowledge and craftsmanship behind the Mexican producers we want to celebrate."}
        </p>
        ...
        <div className="melodisa-actions melodisa-actions--center">
          <Button
            ...
            navigate={navigate}
          >
            {isMexico ? "Ver nuestra miel" : "Explore Benditos honey"}
          </Button>
...
function AboutPage({ region, navigate }) {
  const isMexico = region === "mx";

  return (
    <main>
      <PageHero
        eyebrow={isMexico ? "Nuestra historia" : "Our story"}
        title="Benditos"
        copy={
          isMexico
      ...
      <section className="story section">
        ...
        <div className="story__copy">
          <p className="eyebrow">
            {isMexico ? "Nacido entre cocinas" : "Born in kitchens"}
          </p>
          ...
          <Button
            ...
            navigate={navigate}
          >
            {isMexico ? "Ver lo que cocinamos" : "See what we cook"}
          </Button>
        </div>
      </section>
    </main>
...
function VisitPage({ region, navigate }) {
  const isMexico = region === "mx";

  return (
    <main>
      <PageHero
        eyebrow={isMexico ? "Próximamente" : "Coming soon"}
        title={isMexico ? "Visítanos en Mexicali" : "Find us in London"}
        copy={
          isMexico
      ...
      <section className="visit-grid section">
        <article>
          <p className="eyebrow">{isMexico ? "Ubicación" : "Location"}</p>
          <h2>
            {isMexico
              ? "Mexicali, Baja California"
              : "London, United Kingdom"}
          ...
          <p>
            {isMexico ? "Dirección por anunciar." : "Location to be announced."}
          </p>
        </article>

        <article>
          <p className="eyebrow">
            {isMexico ? "Horario" : "Opening hours"}
          </p>
          <h2>{isMexico ? "Próximamente" : "Coming soon"}</h2>
          <p>
            {isMexico
        ...
        <article>
          <p className="eyebrow">{isMexico ? "Novedades" : "Updates"}</p>
          <h2>{isMexico ? "Sé de los primeros." : "Be first to know."}</h2>

          <Button
            to={routePath(region, "contact")}
            navigate={navigate}
          >
            {isMexico ? "Quiero enterarme" : "Keep me posted"}
          </Button>
        </article>
      </section>
    </main>
...
function ContactPage({ region }) {
  const isMexico = region === "mx";

  return (
    <main>
      <PageHero
        eyebrow={isMexico ? "Hablemos" : "Let’s talk"}
        title={isMexico ? "Contacto" : "Contact"}
        copy={
          isMexico
      ...
      <section className="contact section">
        <div className="contact__intro">
          <p className="eyebrow">
            {isMexico ? "Escríbenos" : "Get in touch"}
          </p>
        ...
        >
          <input
            type="hidden"
            name="region"
            value={isMexico ? "Mexicali" : "London"}
          />

          <label>
            {isMexico ? "Nombre" : "Name"}
            <input
              type="text"
              name="name"
              required
          ...
          <label>
            {isMexico ? "Correo electrónico" : "Email"}
            <input
              type="email"
              name="email"
              required
          ...
          <button
            ...
            type="submit"
          >
            {isMexico ? "Enviar mensaje" : "Send message"}
          </button>
        </form>
      </section>
    </main>
...
function NotFound({ region, navigate }) {
  const isMexico = region === "mx";
  ...
  return (
    <main className="not-found section">
      ...
      <Button
        ...
        navigate={navigate}
      >
        {isMexico ? "Volver al inicio" : "Back home"}
      </Button>
    </main>
  );
}
...
export default function App() {
  ...
  const pages = {
    ...
    "/menu": <MenuPage region={route.region} />,
    "/honey": (
      <HoneyPage
        region={route.region}
        navigate={navigate}
    ...
    "/contact": <ContactPage region={route.region} />,
  };
